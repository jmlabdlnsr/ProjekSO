...
SJF Scheduling GUI — single window with 3 columns:
...
class Process:
    ...
    def __post_init__(self):
        ...
        self.sort_index = (self.arrival, self.burst, self.pid)

# ---------------- Scheduling logic (SJF) ----------------
def simulate_sjf_processes(proc_tuples: List[Tuple[str,int,int]], preemptive: bool=False):
    """
    Input: list of (pid, arrival, burst)
    Returns:
        - procs: list of Process objects with metrics filled
        - gantt_segments: list of (start, end, pid)
        - total_time: simulation end time
    """
    # Create Process objects (deep-ish copy)
    procs = [Process(pid, at, bt) for (pid, at, bt) in proc_tuples]
    procs_sorted = sorted(procs, key=lambda p: (p.arrival, p.burst, p.pid))
    n = len(procs)
    time_now = 0
    completed = 0
    ready: List[Process] = []
    ...
    while completed < n:
        ...
        # choose by shortest remaining burst (for preemptive) or shortest burst (for non-preemptive)
        if preemptive:
            # sort by remaining then arrival then pid
        ...
        while ran < run_len:
            ...
            if preemptive and cur.remaining > 0 and ready:
                # re-evaluate ready queue: if someone with smaller remaining arrived, preempt
                ready.sort(key=lambda p: (p.remaining, p.arrival, p.pid))
...
root = Tk()
root.title("SJF Scheduler - Kelas A ")
...
Label(left_frame, text="➤ SJF Scheduler (SRTF + Non-preemptive)", font=("Segoe UI", 14, "bold"), bg=APP_BG, fg=TEXT_COLOR).pack(pady=8)

# Input entries (no Priority)
...
chk_preempt = Checkbutton(ctrl_frame, text="Preemptive (SRTF)", variable=preempt_var, bg=APP_BG)
...
col_widths = {"PID":48,"AT":36,"BT":36,"ST":48,"CT":48,"TAT":48,"WT":48}
for c in res_cols:
    res_tree.heading(c, text=c)
    res_tree.column(c, width=col_widths.get(c,40), anchor=CENTER)
...
for c,w in zip(cols, (140,140,140)):
...
def load_sample():
    global process_list
    # sample demonstrates preemption cases
    ...
    try:
        with open(path, newline='') as f:
            ...
            if not rows:
                ...
                return
            # accept either header row or direct numeric rows
            start_idx = 0
            # detect header if first row not numeric in first two cells
            first = rows[0]
            if not first:
                messagebox.showerror("CSV Error", "Malformed CSV")
                return
            # if the first row contains non-integers in the first two columns, skip it
            try:
...
def render_3d_gantt(gantt_segments: List[Tuple[int,int,str]], procs_meta: List[Process], total_time: int):
    ...
    ax.set_title("3D Gantt (SJF Scheduling)", color=TEXT_COLOR)
...
def run_sim():
    ...
    def worker():
        procs_meta, gantt, total_time = simulate_sjf_processes(procs_snapshot, preemptive=preempt)
...
def save_gantt_png():
    # Ask path
    p = filedialog.asksaveasfilename(defaultextension=".png", filetypes=[("PNG","*.png")], initialfile="gantt_7747FD_sjf.png")
    if not p:
        return
